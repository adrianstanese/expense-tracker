...
/* ═══════════════════════════════════════════════════════════════
   EXPENSE TRACKER — Adrian & Denitsa
   Design: Team Vacation Planner glass-morphism language
   Font: DM Sans · Glass blur · Gradient CTAs · 3 themes
   Backend: Neon Postgres via /api/expenses
   ═══════════════════════════════════════════════════════════════ */
...
const TH: Record<string, any> = {
  ...
  pink: {
    ...
    inputBg:"rgba(255,255,255,0.6)",inputBd:"rgba(214,36,110,0.12)",
  },
};
...
type Expense = { id:string; username:string; amount:number; description:string; created_at:string };

// ─── API helpers ─────────────────────────────────────────────
async function fetchExpenses(): Promise<Expense[]> {
  const r = await fetch("/api/expenses"); return r.json();
}
...
async function clearAll() {
  await fetch("/api/expenses", { method:"DELETE", headers:{"Content-Type":"application/json"}, body:JSON.stringify({id:"__ALL__"}) });
}
...
export default function ExpenseTracker() {
  ...
  // Load on mount + poll every 15s for live sync
  useEffect(() => {
  ...
  const addExpense = useCallback(async () => {
    ...
    const entry = {
      ...
      username: user, amount: Math.round(v*100)/100, description: desc.trim(),
    };
    try {
      await postExpense(entry);
      const fresh = await fetchExpenses();
      setExpenses(fresh);
      setAmount(""); setDesc(""); setToast(true);
  ...
  const glass = (extra: any={}) => ({
    ...
    borderRadius:G.r, backdropFilter:G.blur, WebkitBackdropFilter:G.blur,
    boxShadow:th.cardS, ...extra,
  });
          ...
          <div style={{ display:"flex",gap:16,justifyContent:"center" }}>
            {[{n:"Adrian",g:th.adG,c:th.adC},{n:"Denitsa",g:th.deG,c:th.deC}].map(({n,g,c})=>(
              <button key={n} onClick={()=>{setUser(n);setView("add");}}
                style={{
                  flex:1,padding:"24px 16px",borderRadius:G.r,
                  background:th.gbg, border:`1.5px solid ${th.gbd}`,
                  ...
                  transition:"all .25s",fontFamily:F,
                }}
                onMouseEnter={(e: any)=>{e.currentTarget.style.borderColor=c;e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow=`0 8px 28px ${c}22`;}}
                onMouseLeave={(e: any)=>{e.currentTarget.style.borderColor=th.gbd;e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none";}}
              >
                <div style={{ width:60,height:60,borderRadius:"50%",background:g,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,color:"#fff",fontWeight:800,boxShadow:`0 4px 16px ${c}33`,letterSpacing:-.5 }}>
                  {n[0]}
                </div>
            ...
            {Object.entries(TH).map(([k,v]: any)=>(
              <button key={k} onClick={()=>setTk(k)}
                style={{
                  ...
                  border:tk===k?`2px solid ${th.ac}`:`1.5px solid ${th.bd}`,
                  background:k==="light"?"#f0f4fa":k==="dark"?"#111828":"#fef1f7",
                  cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",
                  transition:"all .2s",boxShadow:tk===k?`0 0 0 3px ${th.acS}`:"none",
                }}>{v.ic}</button>
            ))}
          </div>
        </div>
  ...
  return (
    <div style={{ fontFamily:F, background:th.bg, minHeight:"100vh", transition:"all .4s" }}>
      ...
      <header style={{
        ...
        borderBottom:`1px solid ${th.bd}`, boxShadow:th.hdrS,
        padding:"0 24px", height:58, display:"flex", alignItems:"center",
      }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          <div style={{ width:34,height:34,borderRadius:10,background:th.acG,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:th.acSh }}>
        ...
        <div style={{ display:"flex",gap:2,background:th.gbg,borderRadius:G.rS,padding:3,backdropFilter:G.blurS,WebkitBackdropFilter:G.blurS,border:`1px solid ${th.bd2}`,marginRight:14 }}>
          {[{k:"add",l:"Add",d:"M12 4v16m8-8H4"},{k:"list",l:"History",d:"M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"}].map(n=>(
            <button key={n.k} onClick={()=>setView(n.k)}
              style={{
                display:"flex",alignItems:"center",gap:5,padding:"6px 14px",borderRadius:G.rXs,
                border:"none",cursor:"pointer",fontSize:13,fontWeight:view===n.k?700:500,
                background:view===n.k?th.card:"transparent", color:view===n.k?th.ac:th.t3,
                boxShadow:view===n.k?th.cardS:"none", transition:"all .2s",
              }}>
              <Ic d={n.d} s={15} c={view===n.k?th.ac:th.t3} sw={view===n.k?2.5:1.8}/>{n.l}
            </button>
          ))}
        </div>
        ...
        <div style={{
          ...
          padding:"4px 12px 4px 5px",borderRadius:20,
          background:th.gbg,border:`1px solid ${th.bd}`,
          backdropFilter:G.blurS,WebkitBackdropFilter:G.blurS,
        }}>
          ...
          <button onClick={()=>setUser(null)} style={{ background:"none",border:"none",cursor:"pointer",color:th.t3,fontSize:16,lineHeight:1,padding:0,marginLeft:2 }} title="Switch">×</button>
        </div>

        {/* Theme dots */}
        <div style={{ display:"flex",gap:5,marginLeft:12 }}>
          {Object.entries(TH).map(([k,v]: any)=>(
            <button key={k} onClick={()=>setTk(k)} style={{
              width:26,height:26,borderRadius:"50%",
              border:tk===k?`2px solid ${th.ac}`:`1px solid ${th.bd}`,
              background:k==="light"?"#f0f4fa":k==="dark"?"#111828":"#fef1f7",
      ...
      <div style={{ maxWidth:640,margin:"0 auto",padding:"28px 20px" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:28 }}>
          {[
            {l:"Adrian",v:totA,c:th.adC,g:th.adG,n:expenses.filter(e=>e.username==="Adrian").length},
            {l:"Denitsa",v:totD,c:th.deC,g:th.deG,n:expenses.filter(e=>e.username==="Denitsa").length},
            {l:"Total",v:totAll,c:th.ac,g:th.acG,n:expenses.length},
          ].map((s,i)=>(
            <div key={i} style={{
        ...
        {view==="add" && (
          <div style={{ animation:"fadeUp .35s ease" }}>
            ...
            <p style={{ fontSize:13,color:th.t2,margin:"0 0 20px" }}>Recording as <strong style={{color:user==="Adrian"?th.adC:th.deC}}>{user}</strong></p>

            {toast && (
              <div style={{
                ...glass({padding:"12px 16px",marginBottom:16,borderColor:th.ok}),
                color:th.ok,fontSize:13,fontWeight:600,display:"flex",alignItems:"center",gap:8,
                animation:"slideDown .3s ease",
              }}>
                <Ic d="M20 6L9 17l-5-5" s={18} c={th.ok} sw={2.5}/> Expense added!
              </div>
            )}
            ...
            <button onClick={addExpense} disabled={!amount||!desc.trim()||saving}
              style={{
                width:"100%",padding:"15px 24px",borderRadius:G.r,
                background:(!amount||!desc.trim())?th.bd:th.acG,
                color:(!amount||!desc.trim())?th.t3:"#fff",
                border:"none",cursor:(!amount||!desc.trim()||saving)?"default":"pointer",
                fontSize:15,fontWeight:700,fontFamily:F,letterSpacing:.2,
                boxShadow:(!amount||!desc.trim())?"none":th.acSh,
                transition:"all .25s", opacity:saving?.6:1,
              }}>
            ...
            {expenses.length>0 && (
              <div style={{ marginTop:28 }}>
                ...
                {expenses.slice(0,5).map((e,i)=>(
                  <div key={e.id} style={{
                    ...glass({padding:"10px 14px",marginBottom:6}),
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      ...
                      <div style={{ fontSize:10,color:th.t3 }}>{fD(e.created_at)} · {fT(e.created_at)}</div>
                    </div>
        ...
        {view==="list" && (
          <div style={{ animation:"fadeUp .35s ease" }}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,marginBottom:20 }}>
              <div>
                <h2 style={{ fontSize:18,fontWeight:800,color:th.tx,margin:0,letterSpacing:-.3 }}>All Expenses</h2>
              ...
              <div style={{ display:"flex",gap:3,background:th.gbg,borderRadius:G.rS,padding:3,backdropFilter:G.blurS,WebkitBackdropFilter:G.blurS,border:`1px solid ${th.bd2}` }}>
                {[{k:"all",l:"All"},{k:"Adrian",l:"Adrian"},{k:"Denitsa",l:"Denitsa"}].map(f=>(
                  <button key={f.k} onClick={()=>setFilter(f.k)} style={{
                    padding:"6px 13px",borderRadius:G.rXs,border:"none",
                    background:filter===f.k?th.card:"transparent",
                    color:filter===f.k?th.ac:th.t3,fontWeight:filter===f.k?700:500,
                    fontSize:12,cursor:"pointer",fontFamily:F,
                    boxShadow:filter===f.k?th.cardS:"none",transition:"all .15s",
                  }}>{f.l}</button>
                ))}
              </div>
            </div>
            ...
            {loading ? (
              <div style={{ textAlign:"center",padding:40,color:th.t3 }}>Loading...</div>
            ) : filtered.length===0 ? (
              <div style={{ ...glass({textAlign:"center" as const,padding:"48px 24px"}) }}>
                ...
                <div style={{ fontSize:13,color:th.t3,marginTop:4 }}>Add your first expense to get started</div>
              </div>
            ) : (
              <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                {filtered.map((e,i)=>(
                  <div key={e.id} style={{
                    ...
                    animation:`fadeUp .3s ease ${Math.min(i,10)*0.03}s both`,
                    transition:"all .2s",
                  }}
                    onMouseEnter={(ev: any)=>ev.currentTarget.style.borderColor=th.gbh}
                    onMouseLeave={(ev: any)=>ev.currentTarget.style.borderColor=th.gbd}
                  >
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ fontSize:14,fontWeight:600,color:th.tx,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{e.description}</div>
            ...
            {expenses.length>0 && (
              <div style={{ marginTop:28,textAlign:"center" }}>
                <button onClick={handleClear}
                  style={{ padding:"8px 20px",borderRadius:G.rS,background:th.erBg,border:`1px solid ${th.er}22`,color:th.er,fontWeight:600,fontSize:12,cursor:"pointer",fontFamily:F,transition:"all .15s" }}
                  onMouseEnter={(e: any)=>e.currentTarget.style.background=th.er+"18"}
                  onMouseLeave={(e: any)=>e.currentTarget.style.background=th.erBg}
